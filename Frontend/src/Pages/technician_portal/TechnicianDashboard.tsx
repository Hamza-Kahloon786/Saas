...
//     const response = await api.post(`/technician-portal/jobs/${job.id}/generate-invoice`, {})
//     return response.data
//   },
//   onSuccess: (data) => {
...
//   console.log('Generating invoice for job:', job.id)
//   generateInvoiceMutation.mutate(job)
// }

// const sendInvoiceToCustomer = async (job) => {
//   try {
//     const response = await api.post(`/technician-portal/jobs/${job.id}/send-invoice`)
//     toast.success('Invoice sent to customer!')
    
//     const paymentLink = `http://localhost:5173/customer-portal/payments/${response.data.invoice_id}`
...
//   },
//   onSuccess: async (data, variables) => {
//     console.log('Photo upload response:', data); // Debug log
...
//     // Try to get address from job.location
//     if (job.location) {
...
//     if (!address && job.customer && job.customer.address) {
//       address = job.customer.address
//     }
    
//     console.log('Navigation data:', { address, coordinates })
...
//       toast.error('No location information available for this job')
//       return
//     }
  ...
  const sendInvoiceMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.payment_link || data.invoice_id) {
        const paymentLink = data.payment_link || `http://localhost:5173/customer-portal/payments/${data.invoice_id}`
        console.log('Customer payment link:', paymentLink)
